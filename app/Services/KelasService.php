...
class KelasService
{
  ...
  public function syncKelasTK()
  {
    ...
    $kelases = $this->kelasRepository->getKelasTK($tahunActive->name)->pluck('level');
    foreach ($kelases as $kelas) {
      $existingKelases = $this->kelasRepository->getKelasByJenjangAndSchool($jenjang->id, $school->id)->pluck('name')->all();
      // dd($existingKelases);
      ...
      if (!in_array($kelas->kelas, $existingKelases) && !in_array($this->getKelasName($kelas->kelas), $existingKelases)) {
        ...
        $this->kelasRepository->insertKelas(
          [
            'jenjang_id' => $jenjang->id,
            'school_id' => $school->id,
            'name' => $this->getKelasName($kelas->kelas),
            'code' => $this->getKelasName($kelas->kelas)
          ]
        );
      }
    }

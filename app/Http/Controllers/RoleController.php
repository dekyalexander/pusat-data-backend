...
use App\Services\EmployeeCareerService;
...
class RoleController extends Controller
{
    ...
    protected $employeeCareerService;
    protected $roleUserService;

    public function __construct(RoleService $roleService, EmployeeCareerService $employeeCareerService, RoleUserService $roleUserService)
    {
    ...
    {
        try {
            $query = $this->roleService->getUsersOfRole($request->role_id, $request->rowsPerPage);
            return response($query);
        } catch (\Exception $e) {
            return response(['error' => $e->getMessage(), 'message' => 'failed get data']);
        }
    ...
    public function addUserByPosition(Request $request)
    {
        $filters = $request->all();
        $data = $this->employeeCareerService->getCareerByOption($filters);
        ...
        try {
            ...
            foreach ($data as $key) {
                $filter = [
                    ...
                    "userId" => $key["user"]["user"]["id"]
                ];

                $existingUser = $this->roleUserService->getRoleUserByFilter($filter)->pluck('id');
                // dd($existingUser);
                if (count($existingUser) === 0) {
                    $this->roleService->addUserOfRole($filters["roleId"], [$key["user"]["user"]["id"]]);
                }
            }
            DB::commit();
            return response([

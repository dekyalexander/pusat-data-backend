  ...
  public function getForOptions($request)
  {
    return $this->role->select('id', 'name')
      ->when(isset($request['not_id']), function ($query) use ($request) {
        return $query->orWhere('id', '<>', $request['not_id']);
      })->when(isset($request['positionId']), function ($query) use ($request) {
        return $query->orWhere('employee_position_id', '=', $request['positionId']);
      })->when(isset($request['unitId']), function ($query) use ($request) {
        return $query->orWhere('employee_position_id', '=', $request['unitId']);
      })->when(isset($request['employeeId']), function ($query) use ($request) {
        return $query->orWhere('employee_id', '=', $request['employeeId']);
      })->get();
  }
